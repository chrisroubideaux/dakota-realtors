...
import { FaVideo, FaPaperPlane, FaMapPin, FaBed, FaBath } from 'react-icons/fa';

export default function Messages({ setActiveComponent, currentUserId }) {
  ...
  // Fetch users and agents on component load
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    const messageData = {
      ...
      senderModel: 'User',
      recipientModel: activeRecipient.role === 'User' ? 'User' : 'Agent',
                ...
                >
                  <div
                    className={`flex-shrink-0 avatar avatar-xs me-2 ${
                      msg.sender._id === currentUserId ? 'd-none' : ''
                    }`}
                  >
                    <img
