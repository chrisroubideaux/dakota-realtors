...
export default function Messages({
  ...
  useEffect(() => {
    ...
    fetchMessages();
  }, [currentAgentId, activeRecipient]);
  // Post method
  const sendMessage = async () => {
    ...
    const messageData = {
      sender: { _id: currentAgentId },
      recipient: { _id: activeRecipient._id },
      senderModel: 'Agent',
      recipientModel: activeRecipient.role === 'Agent' ? 'Agent' : 'Admin',
                ...
                >
                  <div
                    className={`flex-shrink-0 avatar avatar-xs me-2 ${
                      msg.sender._id === currentAgentId ? 'd-none' : ''
                    }`}
                  >
                    <img
